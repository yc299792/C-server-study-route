...
{
    if(!subscriberMap_.count(conn))//检查一个连接是否在con-topic中，没有加入进来
    {
...
void PubSub::subscribe(std::shared_ptr<Connection> conn, const std::string &topic)
{
	int groupid = 0;
	srand((int)time(NULL));
    if(subscriberMap_.count(conn))//当一个连接发起订阅就把连接加入到con-topic map中去
    {
        subscriberMap_[conn].insert(topic);//key是conn，value是一个集合存储topic
    }
    else
    {
    ...
    if(!queueMap_.count(topic))//如果没有存储这种消息的队列就创建一个
    {
        //queueMap_[topic]->addConnection(conn);
        queueMap_.insert({topic,std::make_shared<socketx::MessageQueue>()});//创建一个智能指针维护MessageQueue对象
    }
	//添加之前需要设置conn的组id，从已经有的组里随机一个出来
	groupid = rand() % (queueMap_[topic]->getGroupSize()+1);
	//groupid = 0;
	conn->setGroupId(groupid);
	printf("subcriber groupid is %d\n!",groupid);
    queueMap_[topic]->addConnection(conn);//就是topic里去添加一个conn，不管topic存在与否
...
void PubSub::unsubscribe(std::shared_ptr<Connection> conn, const std::string &topic)
{
    ...
    if(queueMap_.count(topic))//从topic-con中移除con
    {
...
{
    if(channelMap_.count(topic))//delete a topic from channelmap
    {
